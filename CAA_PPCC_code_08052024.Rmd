...
plot_panel_1_Rubisco_tax = function(asvtable, meta, filter_group, legend_position){
  ...
  gg_plot_name_vec = c('Thalassiosirales' = 'darkcyan',
                       ...
                       'Phaeocystis' = 'pink3',
                       'Fragilariopsis/Pseudo-nitzschia'= 'cornsilk4',NAMED_GGPLOT_BAR_COLS)
...
plot_panel_2_Rubisco_2 = function(Rubisco_meta, ctd_data){
  ...
  gg_plot_name_vec = c('Thalassiosirales' = 'darkcyan',
                       ...
                       'Micromonas' = 'darkseagreen3',
                       'Phaeocystis' = 'pink3',
                       'Fragilariopsis/Pseudo-nitzschia' = 'cornsilk4')
...
make_pca_plot = function(pca_meta, full_meta, scl, vectors){
  ...
  temp_plot = ggplot(pca_plot) + geom_point(data  = pca_plot, mapping = aes(x = PC1, y = PC2, fill = Glacier_Name, shape = SURF_SCM), size=15, alpha = .8) +
    ...
    xlab((names(pca_plot)[1])) +
    ylab((names(pca_plot)[2])) +
    theme_bw() + 
    theme(
      panel.grid.major = element_blank(),
...
PLOT_DIR = '/Users/patrickwhite/Documents/GitHub/CAA_TWG_2024/CAA_TWG_2024_BL/CAA_PPCC_2024/plot_outputs/' 

# barplot colors 
GGPLOT_BAR_COLS = c('darkcyan','thistle3','darkorange2','#F39B7FB2','cornsilk2','tan1','ivory1','lightsteelblue3','thistle2','mediumaquamarine','palevioletred4','indianred3','darkseagreen3','plum','darkblue','cornsilk4','pink3','moccasin','blue1','gold1','cyan1','tomato2','tomato3','palevioletred1','salmon2','mediumorchid4','palegreen','yellow3','darkorange4','orange3','wheat', 'green','tan') 

# named barplot colors with taxonomic groups
NAMED_GGPLOT_BAR_COLS = c(
  'Thalassiosirales' = 'darkcyan' ,
  'Thalassiosira' = 'thistle3' ,
  'Attheya' = 'darkorange2' ,
  'Chaetocerotales' = '#F39B7FB2' ,
  'Chaetoceros' = 'cornsilk2' ,
  'Other Diatoms' = 'tan1' ,
  'Eutreptiella' = 'ivory1' ,
  'Ochromonas' = 'lightsteelblue3' ,
  'Pedinella' = 'thistle2' ,
  'Chrysochromulina' = 'mediumaquamarine' ,
  'Cymbomonas' = 'palevioletred4' ,
  'Coscinodiscophyceae' = 'indianred3' ,
  'Micromonas' = 'darkseagreen3' ,
  'Rhizosolenia' = 'plum' ,
  'Ochrophyta' = 'darkblue' ,
  'Fragilariopsis' = 'cornsilk4' ,
  'Phaeocystis' = 'pink3' ,
  'Pyramimonas' = 'tan' ,
  'Prymnesiales' = 'blue1' ,
  'Imantonia' = 'gold1' ,
  'Cylindrotheca' = 'cyan1' ,
  'Geminigeraceae' = 'tomato2' ,
  'Teleaulax' = 'tomato3' ,
  'Prymnesiophycidae' = 'palevioletred1' ,
  'Prymnesium' = 'salmon2' ,
  'Florenciella' = 'mediumorchid4' ,
  'Unknown Eukaryota' = 'palegreen' ,
  'Cymbellaceae' = 'yellow3' ,
  'Cymatosiraceae' = 'darkorange4' ,
  'Archaeplastida' = 'orange3' ,
  '<1%' = 'wheat'
)
...
#read in ASV table
ASV_table = read.csv('/Users/patrickwhite/Documents/GitHub/CAA_TWG_2024/CAA_TWG_2024_BL/CAA_PPCC_2024/formated_ASVs_chl_CAA_PPCC_04242024.csv')

#read in taxonomic table
phytoref_table = read.csv('/Users/patrickwhite/Documents/GitHub/CAA_TWG_2024/CAA_TWG_2024_BL/CAA_PPCC_2024/formated_phytoref_chl_CAA_PPCC_04242024.csv')

meta_16s = read.csv('/Users/patrickwhite/Documents/GitHub/CAA_TWG_2024/CAA_TWG_2024_BL/CAA_PPCC_2024/devon_metadata_microbial_2021_5m_meaned_complete_06_05_2024.2.csv')
...
likely_mixo = c("Ochromonas", "Chrysochromulina","Pedinella","Pyramimonas",
                 'Prymnesium','Teleaulax','Cymbomonas')

maybe_mixo = c("Prymnesiales",'Geminigeraceae','Prymnesiophycidae','Imantonia','Florenciella','Micromonas')

unknown = c('Eutreptiella','Ochrophyta','Unknown Eukaryota','Archaeplastida','<1%',"Phaeocystis")

#creating mixotrophic status variables based on above vectors
tax_plot_df$mixtrophy_status = ifelse(tax_plot_df$Genus %in% likely_auto,'Autotroph',
  ifelse(tax_plot_df$Genus %in% likely_mixo,'Mixotroph',
    ifelse(tax_plot_df$Genus %in% maybe_mixo,'Potential mixotroph','Unknown')))

#group by mixotrophic status to recreate varplots groups for plotting
tax_plot_df_mixo = tax_plot_df %>% ungroup() %>% 
  group_by(SAMPLE_ID, mixtrophy_status) %>% 
  dplyr::summarise(percent = sum(percent))

# ad back in sample ID and sample names to mixotrophic plot 
tax_plot_df_mixo = inner_join(tax_plot_df_mixo, (tax_plot_df %>% select(site, SAMPLE_ID, Sample_Name_Final) %>% dplyr::distinct()), by = 'SAMPLE_ID')

#factor mixotrophy names for plotting
tax_plot_df_mixo$mixtrophy_status = factor(tax_plot_df_mixo$mixtrophy_status, 
                                           levels = c('Unknown','Autotroph','Potential mixotroph','Mixotroph'))

col_vec = c("grey", "lightsalmon", "lightskyblue1", "lightskyblue3")

#get some gneeral information on prevalence of mixtoprhy across sites
tax_plot_df_mixo$site
tax_plot_df_mixo%>% filter(site == 'Weak tidewater glacier influence') %>% 
  group_by(mixtrophy_status) %>% 
  dplyr::summarise(mean(percent))

unique(tax_plot_df_mixo$site)
tax_plot_df_mixo %>% filter(site == 'No tidewater glacier') %>% 
  group_by(mixtrophy_status) %>% 
  dplyr::summarise(mean(percent))

tax_plot_df_mixo %>% filter(site == 'Strong tidewater glacier influence') %>% 
  group_by(mixtrophy_status) %>% 
  dplyr::summarise(mean(percent))
...
tax_plot_df_mixo %>% filter(site %in% c('No tidewater glacier','Weak tidewater glacier influence')) %>% filter(mixtrophy_status %in% c('Mixotroph', 'Potential mixotroph')) %>% dplyr::summarise(sum_perc = sum(percent)) %>% ungroup() %>% dplyr::summarise(mean_perc = mean(sum_perc),med_perc = median(sum_perc))

tax_plot_df_mixo %>% filter(grepl('Starnes2', Sample_Name_Final)) %>% filter(mixtrophy_status %in% c('Mixotroph', 'Potential mixotroph')) %>% dplyr::summarise(sum_perc = sum(percent)) %>% dplyr::summarise(mean_perc = mean(sum_perc),med_perc = median(sum_perc))

tax_plot_df_mixo %>% filter(site %in% c('Strong tidewater glacier influence')) %>% filter(mixtrophy_status %in% c('Mixotroph', 'Potential mixotroph')) %>% dplyr::summarise(sum_perc = sum(percent)) %>% ungroup() %>% dplyr::summarise(mean_perc = mean(sum_perc),med_perc = median(sum_perc))

tax_plot_df_mixo %>% filter(site %in% c('Strong tidewater glacier influence')) %>% filter(mixtrophy_status %in% c('Mixotroph', 'Potential mixotroph')) %>% dplyr::summarise(sum_perc = sum(percent)) %>% ggplot(aes(x = sum_perc)) + geom_density()

mixotrophy_plot = ggplot(tax_plot_df_mixo, aes(x = (Sample_Name_Final), y = percent, fill = mixtrophy_status)) + # make plot
...
mixotrophy_plot = mixotrophy_plot + theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 10))

gridder = cowplot::plot_grid(rel_abund_plot,mixotrophy_plot, nrow = 2, rel_heights = c(1, .5))
...
gridder = cowplot::plot_grid(rel_abund_plot,chae_main_plot,mixotrophy_plot, nrow = 3, rel_heights = c(1,.15,.5))
gridder
...
chla_tracers = read.csv('/Users/patrickwhite/Documents/GitHub/CAA_TWG_2024/CAA_TWG_2024_BL/CAA_PPCC_2024/chla_tracer_data_rubisco_CAA_PPCC.csv')

#read in rubisco and assocaited metadata
Rubisco_data = read.csv('/Users/patrickwhite/Documents/GitHub/CAA_TWG_2024/CAA_TWG_2024_BL/CAA_PPCC_2024/Rubisco_data_meta_CAA_PPCC_06052024.csv')
...
ggsave(paste0(PLOT_DIR,'Rubisco_fig_all_nuts_',CURRENT_DATE,'.',file_extension),full_Rubisco_plot, 
       device = file_extension, width = 4000, height = 4000, units = 'px')  
...
vars_all = c('SAMPLE_ID','landtype','Temperature', 'Salinity','Turbidity', 'Chlorophyl a','N2', 'Dissolved O2','SiO4','PO4','NO3','NH3')

# Select relevant columns from meta_16s then pivot the data from wide to long format
long = meta_16s %>%
  select(vars_all, Glacier_Name) %>%
  select(-SAMPLE_ID) %>%
...
name_change = c('Dissolved O2' = 'Dissolved oxygen',
  'NO3' = 'Nitrate',
  'SiO4' = 'Silicate',
  'NH3' ='Ammonia',
  'PO4' = 'Phosphate',
  ...
  'Chlorophyl a' = 'Chlorophyll a',
  'N2' = 'N2')
long$name = unname(name_change[long$name])

# Convert 'name' to a factor with predefined levels
long$name = factor(long$name, levels = c('Temperature', 'Salinity','Turbidity', 'N2', 'Dissolved oxygen','Chlorophyll a','Silicate','Phosphate','Nitrate','Ammonia'))

# Create a boxplot using ggplot
bp = ggplot(long, aes(y = value, x = Glacier_Name, fill = Glacier_Name)) +
  geom_boxplot() +
...
file_extension = 'png'
ggsave(paste0(PLOT_DIR,'box_plot_chem_phys_Supp_',CURRENT_DATE,file_extension), bp, 
       device = file_extension, width = 3600, height = 3600, units = 'px')  
```
